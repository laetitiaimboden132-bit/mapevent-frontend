...
Fixed: pagination starts at page=1, not 0.
Areas: Paris (44), Geneva (60), Nantes (155) + scan for more.
"""
...
def classify_electro(title, artists=""):
    ...
    subgenre_map = {
        ...
        "House": ["house music", "deep house", "funky house", "afro house", "soulful house"],
...
# ==========================================================================
# 2. Known areas + scan for more
# ==========================================================================
print("\n" + "=" * 70)
print("RESIDENT ADVISOR - Areas")
print("=" * 70)

# Already known
known_areas = [
    {"id": 44, "name": "Paris", "country": "FR"},
    {"id": 60, "name": "Geneva", "country": "CH"},
    {"id": 155, "name": "Nantes", "country": "FR"},
]

# Quick scan for more CH/FR areas (100-250 range, we already scanned 1-100)
ch_keywords = ["zurich", "zürich", "bern", "basel", "lausanne", "lucerne", "luzern", "switzerland"]
fr_keywords = ["lyon", "marseille", "bordeaux", "strasbourg", "grenoble", "toulouse", "lille", "nice", "montpellier"]

for area_id in range(100, 300):
    if area_id == 155:  # Already found
        continue
    try:
        ...
        if r.status_code == 200:
            ...
            if events and total > 3:
                ...
                if is_ch or is_fr:
                    ...
                    known_areas.append({"id": area_id, "name": area_name, "country": country})
                    print(f"  NEW Area {area_id}: {area_name} ({country}) - {total} events")
    except:
        pass
...
print(f"\nTotal areas: {len(known_areas)}")
for a in known_areas:
    print(f"  {a['country']} | {a['name']} (ID={a['id']})")


# ==========================================================================
# 3. Fetch events from all areas
# ==========================================================================
print("\n" + "=" * 70)
print("FETCH EVENTS")
print("=" * 70)
...
for area in known_areas:
    ...
    while page <= 6:  # Max 6 pages = 300 events per area
        try:
            r = requests.post("https://ra.co/graphql", json={
                "query": """
                {
            ...
            for item in events:
                ...
                # Verify coordinates are plausible for CH/FR
            ...
            print(f"    Page {page}: +{len(events)} recus, {area_count} nouveaux pour {area_name}")
...
print(f"\nTotal RA events uniques: {len(ra_events)}")
...
print(f"A importer: {len(ra_events)}")
...
for i in range(0, len(ra_events), 10):
    ...
    try:
        ...
        if r.status_code in (200, 201):
            ...
            failed += len(batch)
            print(f"  [{i}/{len(ra_events)}] ERREUR {r.status_code}: {r.text[:150]}")
    except Exception as e:
        failed += len(batch)
        print(f"  [{i}/{len(ra_events)}] ERREUR: {e}")
    time.sleep(0.3)
...
for e in ra_events:
    ...
    if "Paris" in desc or "paris" in e.get("location", "").lower():
        regions["Paris"] = regions.get("Paris", 0) + 1
    elif "Geneva" in desc or "genev" in e.get("location", "").lower():
        regions["Geneva"] = regions.get("Geneva", 0) + 1
    elif "Nantes" in desc or "nantes" in e.get("location", "").lower():
        regions["Nantes"] = regions.get("Nantes", 0) + 1
    else:
        regions["Autre"] = regions.get("Autre", 0) + 1

print("\n  Par region:")
for region, cnt in sorted(regions.items(), key=lambda x: -x[1]):
    print(f"    {region}: {cnt}")
