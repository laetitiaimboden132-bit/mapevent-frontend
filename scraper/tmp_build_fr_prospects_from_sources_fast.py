...
for it in data:
    ...
    if not u: continue
    if ('france' in a) or any(x in c for x in cities): fr_urls.append(u)
fr_urls=list(dict.fromkeys(fr_urls))[:1500]
print('urls candidate', len(fr_urls), flush=True)
...
def one(u):
    try:
        ...
        for e in pat.findall(r.text):
            ...
            if not dom.endswith('.fr'): continue
...
rows=[]; seen=set(); done=0
with cf.ThreadPoolExecutor(max_workers=40) as ex:
    for arr in ex.map(one, fr_urls):
        done += 1
        if done % 200 == 0:
            print('done', done, 'found', len(seen), flush=True)
        for e,u,dom in arr:
            if e in seen: continue
            seen.add(e)
            rows.append({'email':e,'source_url':u,'host':dom})

rows.sort(key=lambda r:(r['host'], r['email']))
out=r"c:\MapEventAI_NEW\frontend\scraper\prospects_france_from_source_urls.json"
with open(out,'w',encoding='utf-8') as f: json.dump(rows,f,ensure_ascii=False,indent=2)
print('new_fr_emails', len(rows), flush=True)
