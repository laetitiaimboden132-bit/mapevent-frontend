/**
 * @fileoverview Script de diagnostic simplifié pour vérifier le username
 * Usage: Copier-coller TOUT le contenu dans la console du navigateur
 */
...
(function() {
  ...
  if (pendingLS) {
    ...
    console.log('  pendingRegisterDataForGoogle.username:', p.username || '❌ MANQUANT');
  } else {
  ...
  if (currentUserLS) {
    ...
    console.log('  currentUser.username:', c.username || '❌ MANQUANT');
  } else {
  ...
  if (window.currentUser) {
    ...
    console.log('  isValid:', window.currentUser.username && 
  ...
  if (accountName) {
    console.log('  Texte affiché:', accountName.textContent);
  } else {
  ...
  if (pendingLS) {
    const p = JSON.parse(pendingLS);
    if (p.username && p.username !== 'null' && !p.username.includes('@')) {
      found = true;
  ...
  console.log('\n💡 Commandes utiles:');
  console.log('  - window.updateAccountBlockLegitimately()');
  console.log('  - getUserDisplayName(window.currentUser)');
  console.log('  - localStorage.getItem("pendingRegisterDataForGoogle")');
})();
