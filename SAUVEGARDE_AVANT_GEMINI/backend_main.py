...
def create_app():
    ...
    def oauth_google():
        ...
        try:
            ...
            # Construire l'objet adresse postale complet si disponible
            postal_address_obj = None
            if postal_address_db and postal_address_db.strip():
                postal_address_obj = {
                    'address': str(postal_address_db) if postal_address_db else '',
            ...
            # CORRECTION CRITIQUE : Créer user_data avec TOUS les champs sérialisables
            # S'assurer que tous les champs sont des types JSON valides (str, int, bool, dict, None)
            # Convertir created_at en string AVANT de créer le dict
            created_at_str = None
            if created_at:
                try:
            ...
            # S'assurer que postal_address_obj est bien un dict ou None (pas un objet complexe)
            postal_address_clean = None
            if postal_address_obj:
                try:
                    # Tester la sérialisation
                    json.dumps(postal_address_obj, ensure_ascii=False, default=str)
                    postal_address_clean = postal_address_obj
                except:
                    # Si échec, créer un dict propre
                    postal_address_clean = {
                        'address': str(postal_address_obj.get('address', '')) if isinstance(postal_address_obj, dict) else str(postal_address_obj),
                        'city': str(postal_address_obj.get('city', '')) if isinstance(postal_address_obj, dict) else '',
                        'zip': str(postal_address_obj.get('zip', '')) if isinstance(postal_address_obj, dict) else '',
                        'country': str(postal_address_obj.get('country', 'CH')) if isinstance(postal_address_obj, dict) else 'CH'
                    }
            ...
            user_data = {
                ...
                'name': str(f"{first_name} {last_name}".strip() if first_name and last_name else username) if username else '',
                'firstName': str(first_name) if first_name else '',
                'lastName': str(last_name) if last_name else '',
                'subscription': str('vip_plus' if is_director else (subscription or 'free')),
                'role': str('director' if is_director else (role or 'user')),
                'avatar': str(avatar_emoji) if avatar_emoji else '👤',
                'avatarDescription': str(avatar_description) if avatar_description else '',
                'profilePhoto': str(profile_photo_url) if profile_photo_url else (str(avatar_emoji) if avatar_emoji else '👤'),
                'profile_photo_url': str(profile_photo_url) if profile_photo_url else '',
                'postalAddress': postal_address_clean,  # Objet dict propre ou None
                'postal_address': postal_address_clean,  # Alias pour compatibilité
                'createdAt': created_at_str,  # String ou None
                'hasPassword': bool(has_password),
                'hasPostalAddress': bool(has_postal_address)
            }
            
            # TEST CRITIQUE : Vérifier que user_data est sérialisable AVANT de continuer
            try:
                test_user_data_json = json.dumps(user_data, ensure_ascii=False, default=str)
                logger.info(f"✅ user_data sérialisable en JSON ({len(test_user_data_json)} caractères)")
            except Exception as test_error:
                logger.error(f"❌ ERREUR CRITIQUE: user_data n'est PAS sérialisable: {test_error}")
                import traceback
                logger.error(traceback.format_exc())
                # Créer une version minimale garantie sérialisable
                user_data = {
                    'id': str(user_id) if user_id else '',
                    'email': str(email) if email else '',
                    'username': str(username) if username else '',
            ...
            # CORRECTION CRITIQUE : Tester la sérialisation JSON AVANT de renvoyer
            try:
                # Tester la sérialisation JSON pour détecter les problèmes
                test_json = json.dumps(user_data, ensure_ascii=False, default=str)
                logger.info(f"✅ user_data sérialisable en JSON ({len(test_json)} caractères)")
                logger.info(f"🔍 user_data keys: {list(user_data.keys())}")
                logger.info(f"🔍 user_data sample: id={user_data.get('id')}, username={user_data.get('username')}, email={user_data.get('email')}")
            except Exception as json_error:
                logger.error(f"❌ Erreur sérialisation user_data: {json_error}")
                import traceback
                logger.error(traceback.format_exc())
                # En cas d'erreur, créer une version minimale mais fonctionnelle
                user_data = {
                    'id': str(user_id) if user_id else '',
                    'email': str(email) if email else '',
                    'username': str(username) if username else '',
            ...
            # CORRECTION CRITIQUE : Sérialiser COMPLÈTEMENT avec json.dumps et utiliser make_response
            # pour garantir que Flask test client peut correctement sérialiser la réponse
            try:
                # Sérialiser user_data en JSON string puis re-parser pour garantir un dict propre
                user_data_json_str = json.dumps(user_data, ensure_ascii=False, default=str)
                user_data_clean = json.loads(user_data_json_str)
                
                # Vérifier que user_data_clean est bien un dict
                if not isinstance(user_data_clean, dict):
                    logger.error(f"❌ user_data_clean n'est pas un dict: {type(user_data_clean)}")
                    user_data_clean = {}
                ...
                # CORRECTION CRITIQUE : Sérialiser la réponse complète AVANT de la renvoyer
                # ET vérifier que user_data_clean est bien un dict sérialisable
                try:
                    # Tester la sérialisation de user_data_clean seul
                    test_user_json = json.dumps(user_data_clean, ensure_ascii=False, default=str)
                    logger.info(f"✅ user_data_clean sérialisable ({len(test_user_json)} caractères)")
                    
                    # Sérialiser la réponse complète
                    response_json_str = json.dumps(response_data, ensure_ascii=False, default=str)
                    logger.info(f"✅ Réponse complète sérialisée ({len(response_json_str)} caractères)")
                    logger.info(f"🔍 user_data keys: {list(user_data_clean.keys())}")
                    logger.info(f"🔍 user_data sample: {json.dumps({k: user_data_clean.get(k) for k in list(user_data_clean.keys())[:5]}, ensure_ascii=False)}")
                    
                    # CORRECTION CRITIQUE : Vérifier que la réponse JSON ne contient pas "[dict"
                    if '[dict' in response_json_str or 'items]' in response_json_str:
                        logger.error(f"❌ ERREUR CRITIQUE: La réponse JSON contient '[dict' ou 'items]' - Flask test client va transformer l'objet")
                        logger.error(f"❌ Réponse JSON (premiers 500 chars): {response_json_str[:500]}")
                        # Forcer la sérialisation en créant un nouveau dict propre
                        user_data_forced = {}
                        for key, value in user_data_clean.items():
                            if isinstance(value, (str, int, float, bool, type(None))):
                                user_data_forced[key] = value
                            elif isinstance(value, dict):
                                user_data_forced[key] = {k: str(v) if not isinstance(v, (str, int, float, bool, type(None))) else v for k, v in value.items()}
                            else:
                                user_data_forced[key] = str(value)
                        
                        response_data_forced = {
                            'user': user_data_forced,
                            'isNewUser': is_new_user,
                            'profileComplete': profile_complete
                        }
                        response_json_str = json.dumps(response_data_forced, ensure_ascii=False, default=str)
                        logger.info(f"✅ Réponse forcée sérialisée ({len(response_json_str)} caractères)")
                    ...
                    logger.error(f"❌ Erreur sérialisation réponse: {serialization_error}")
                    import traceback
                    logger.error(traceback.format_exc())
                    ...
                    return make_response(json.dumps(fallback_response, ensure_ascii=False, default=str), 200, {'Content-Type': 'application/json'})
            except Exception as final_error:
                logger.error(f"❌ Erreur sérialisation response_data: {final_error}")
                import traceback
                logger.error(traceback.format_exc())
                # En cas d'erreur, renvoyer une version minimale
                minimal_response = {
                    'user': {
                        'id': str(user_id) if user_id else '',
                        'email': str(email) if email else '',
                ...
                return Response(
                    json.dumps(minimal_response, ensure_ascii=False, default=str),
    ...
    def oauth_google_complete():
        ...
        try:
            ...
            if not user_row:
                ...
                # CORRECTION CRITIQUE : Récupérer firstName et lastName depuis data
                first_name = data.get('firstName', '').strip()
                last_name = data.get('lastName', '').strip()
            ...
            # CORRECTION CRITIQUE : Sauvegarder firstName et lastName
            first_name = data.get('firstName', '').strip()
            last_name = data.get('lastName', '').strip()
            ...
            if user_row:
                ...
                # Construire l'objet postalAddress
                postal_address_obj = None
                if postal_address_db and postal_address_db.strip():
                    postal_address_obj = {
                        'address': str(postal_address_db),
                        'city': str(postal_city) if postal_city else '',
                        'zip': str(postal_zip) if postal_zip else '',
                        'country': str(postal_country) if postal_country else 'CH'
                    }
                ...
                user_data = {
                    ...
                    'username': str(username) if username else '',
                    'name': str(username) if username else (f"{first_name} {last_name}".strip() if first_name and last_name else ''),
                    'firstName': str(first_name) if first_name else '',
                    'lastName': str(last_name) if last_name else '',
                    'subscription': str(subscription) if subscription else 'free',
                    'role': str(role) if role else 'user',
                    'avatar': str(profile_photo_url) if profile_photo_url else (str(avatar_emoji) if avatar_emoji else '👤'),
                    'avatarDescription': str(avatar_description) if avatar_description else '',
                    'profilePhoto': str(profile_photo_url) if profile_photo_url else (str(avatar_emoji) if avatar_emoji else '👤'),
                    'profile_photo_url': str(profile_photo_url) if profile_photo_url else '',
                    'postalAddress': postal_address_obj,
                    'postal_address': postal_address_obj,  # Alias pour compatibilité
                    'profileComplete': True
                }
                
                # CORRECTION CRITIQUE : Utiliser make_response avec json.dumps pour garantir la sérialisation
                # comme dans oauth_google
                try:
                    response_data = {
                        'success': True,
                        'user': user_data,
                    ...
                    response_data['user'] = user_data_clean
                    
                    # Sérialiser la réponse complète
                    response_json_str = json.dumps(response_data, ensure_ascii=False, default=str)
                    
                    # Vérifier que la réponse JSON ne contient pas "[dict"
                    if '[dict' in response_json_str or 'items]' in response_json_str:
                        logger.error(f"❌ ERREUR CRITIQUE: La réponse JSON contient '[dict' ou 'items]'")
                        # Forcer la sérialisation en créant un nouveau dict propre
                        user_data_forced = {}
                        for key, value in user_data_clean.items():
                            if isinstance(value, (str, int, float, bool, type(None))):
                        ...
                        response_data['user'] = user_data_forced
                        response_json_str = json.dumps(response_data, ensure_ascii=False, default=str)
