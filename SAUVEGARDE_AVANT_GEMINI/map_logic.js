...
async function handleCognitoCallbackIfPresent() {
  ...
  try {
    ...
    try {
      ...
      if (syncResponse.ok) {
        // CORRECTION CRITIQUE : Parser la réponse JSON correctement
        const responseText = await syncResponse.text();
        console.log('🔍 Réponse brute du backend (premiers 500 chars):', responseText.substring(0, 500));
        ...
        try {
          ...
          throw new Error('Réponse backend invalide (pas de JSON)');
        }
        
        // CORRECTION CRITIQUE : Extraire user_data correctement
        // Si syncData.user est une chaîne "[dict - X items]", c'est un problème de sérialisation
        // Dans ce cas, backendUser doit être vide pour forcer l'affichage du formulaire
        let backendUser = {};
        if (syncData.user) {
          if (typeof syncData.user === 'string') {
            // Si c'est "[dict - X items]", c'est un problème de sérialisation Flask
            if (syncData.user.startsWith('[dict') && syncData.user.includes('items]')) {
              console.error('❌ PROBLÈME SÉRIALISATION: syncData.user est "[dict - X items]" - backendUser sera vide');
              ...
              try {
                ...
                console.error('❌ syncData.user est une chaîne mais pas du JSON valide:', syncData.user);
                backendUser = {};
              }
            }
          } else if (typeof syncData.user === 'object' && syncData.user !== null) {
            // Si c'est un objet, créer une copie propre via JSON parse/stringify
            try {
              backendUser = JSON.parse(JSON.stringify(syncData.user));
            } catch (e) {
              console.error('❌ Erreur lors de la copie de syncData.user:', e);
              backendUser = { ...syncData.user }; // Fallback sur spread
            }
          } else {
            backendUser = {};
          }
        ...
        // DEBUG: Afficher la structure complète de syncData
        console.log('✅ Synchronisation backend réussie - DONNÉES COMPLÈTES:', {
          syncData: syncData,
          user: backendUser,
          profileComplete: syncData.profileComplete,
          isNewUser: syncData.isNewUser,
          userKeys: backendUser ? Object.keys(backendUser) : [],
          userString: JSON.stringify(backendUser, null, 2)
        });
        
        console.log('🔍 Extraction données utilisateur (COPIE PROPRE):', {
          username: backendUser.username,
          profile_photo_url: backendUser.profile_photo_url,
          profilePhoto: backendUser.profilePhoto,
          postalAddress: backendUser.postalAddress,
          postal_address: backendUser.postal_address,
        ...
        // Vérifier les données utilisateur essentielles OBLIGATOIRES
        // Utiliser backendUser qui a été extrait ci-dessus
        ...
        // L'adresse postale et l'email sont sauvegardées mais pas obligatoires pour l'affichage
        
        // ============================================
        // LOGIQUE PROFESSIONNELLE MAPEVENT
        // ============================================
        // Le profil est complet SEULEMENT si :
        // 1. Le backend dit profileComplete === true (password + username personnalisé)
        // 2. ET l'utilisateur a un username personnalisé
        // 3. ET l'utilisateur a une photo de profil
        // Si l'une de ces conditions manque, afficher le formulaire
        
        console.log('📊 État utilisateur après validation Google:', {
          isNewUser: isNewUser,
          profileComplete: profileComplete, // ← Source de vérité du backend
        ...
        // Vérifier aussi localStorage comme fallback (seulement pour référence)
        ...
        // CORRECTION CRITIQUE : Vérifier si backendUser est vide (problème de sérialisation)
        // Si backendUser est vide mais profileComplete === true, c'est que le backend a un problème
        // Dans ce cas, on DOIT afficher le formulaire pour permettre à l'utilisateur de compléter son profil
        const isBackendUserEmpty = !backendUser || Object.keys(backendUser).length === 0;
        
        console.log('🔍 DEBUG isBackendUserEmpty:', {
          isBackendUserEmpty: isBackendUserEmpty,
          backendUser: backendUser,
          backendUserKeys: backendUser ? Object.keys(backendUser) : [],
          backendUserType: typeof backendUser,
          backendUserLength: backendUser ? Object.keys(backendUser).length : 0
        });
        
        // Vérifier le backend ET les données essentielles
        // Le profil est complet seulement si le backend dit oui ET que username ET photo sont présents
        // ET que backendUser n'est pas vide (sinon c'est un problème de sérialisation)
        const isProfileCompleteFromBackend = profileComplete === true;
        const hasEssentialData = hasUsername && hasProfilePhoto;
        const isProfileReallyComplete = isProfileCompleteFromBackend && hasEssentialData && !isBackendUserEmpty;
        
        // SI LE PROFIL EST VRAIMENT COMPLET (backend + username + photo + données disponibles), NE JAMAIS AFFICHER LE FORMULAIRE
        if (isProfileReallyComplete) {
          console.log('✅ BACKEND confirme : Profil complet - Connexion directe (PAS de formulaire)', {
            source: 'BACKEND (source de vérité absolue)',
            email: user.email,
            username: user.username,
            userId: user.id,
            profileComplete: profileComplete, // ← Le backend a dit que c'est complet
            message: 'Le backend confirme que le profil est complet (password + username). Connexion directe.'
          });
          
          // Restaurer l'utilisateur depuis le backend (source de vérité)
          // Le backend renvoie TOUTES les données : username, photo, adresse postale complète
          // backendUser a déjà été extrait ci-dessus
          if (backendUser && backendUser.id) {
            
            // Construire l'adresse postale complète depuis le backend
            let postalAddress = null;
            if (backendUser.postalAddress && typeof backendUser.postalAddress === 'object') {
              // Objet complet avec address, city, zip, country
              postalAddress = backendUser.postalAddress;
            } else if (backendUser.postal_address && typeof backendUser.postal_address === 'object') {
              // Alias postal_address
              postalAddress = backendUser.postal_address;
            } else if (backendUser.postalAddress && typeof backendUser.postalAddress === 'string') {
              // String simple
              postalAddress = backendUser.postalAddress;
            } else if (backendUser.postal_address && typeof backendUser.postal_address === 'string') {
              // Alias string
              postalAddress = backendUser.postal_address;
            }
            
            // Photo de profil : priorité profile_photo_url > profilePhoto > avatar
            const profilePhoto = backendUser.profile_photo_url || backendUser.profilePhoto || backendUser.avatar || null;
            
            currentUser = {
              ...currentUser,
              ...backendUser,
              // GARANTIR que l'ID est toujours défini
              id: backendUser.id || backendUser.user_id || currentUser.id || currentUser.sub || null,
              // Photo de profil (priorité absolue au backend)
              profilePhoto: profilePhoto,
              profile_photo_url: backendUser.profile_photo_url || profilePhoto,
              avatar: backendUser.avatar || profilePhoto || '👤',
              // Username personnalisé (priorité absolue au backend) - C'EST LE NOM D'AFFICHAGE
              username: backendUser.username || currentUser.username || null,
              // Prénom et nom (sauvegardés mais pas affichés publiquement)
              firstName: backendUser.firstName || currentUser.firstName || '',
              lastName: backendUser.lastName || currentUser.lastName || '',
              // name = username (pas le nom/prénom Google)
              name: backendUser.username || currentUser.username || '',
              // Adresse postale complète (sauvegardée mais pas affichée publiquement)
              postalAddress: postalAddress || currentUser.postalAddress || null,
              // État de connexion
              isLoggedIn: true,
              provider: 'google',
            ...
            console.log('✅ Utilisateur restauré depuis backend avec TOUTES les données:', {
              id: currentUser.id,
          ...
          showNotification(`✅ Connexion réussie ! Bienvenue ${currentUser.username || 'Utilisateur'}`, "success");
        ...
        // Vérifier localStorage comme fallback (seulement si backend n'a pas confirmé)
        ...
        // CORRECTION CRITIQUE : Si backendUser est vide, on DOIT afficher le formulaire
        // même si profileComplete === true dans localStorage (problème de sérialisation backend)
        // Le localStorage peut être obsolète si le backend a un problème de sérialisation
        if (isBackendUserEmpty) {
          console.log('⚠️ backendUser est vide - FORCER affichage formulaire (ignorer localStorage)', {
            isBackendUserEmpty: isBackendUserEmpty,
            isProfileCompleteFromStorage: isProfileCompleteFromStorage,
          ...
          // Pré-remplir le formulaire avec les données Google si disponibles
          registerData.email = user.email || '';
          registerData.firstName = user.firstName || '';
          registerData.lastName = user.lastName || '';
          registerData.profilePhoto = user.profile_photo_url || user.profilePhoto || user.avatar || '';
          registerData.username = user.username || '';
          
          // Afficher le formulaire d'inscription complet
          let retryCount = 0;
          const maxRetries = 5;
          const displayFormWithRetry = () => {
            if (typeof window.showProRegisterForm === 'function') {
              console.log('✅ Appel de window.showProRegisterForm() (backendUser vide)');
              window.showProRegisterForm();
            } else if (retryCount < maxRetries) {
              console.warn(`⚠️ showProRegisterForm non disponible, nouvelle tentative dans 100ms (tentative ${retryCount + 1}/${maxRetries})`);
              retryCount++;
              setTimeout(displayFormWithRetry, 100);
            } else {
              console.error('❌ Impossible d\'afficher le formulaire d\'inscription après plusieurs tentatives.');
              showNotification("❌ Erreur d'affichage du formulaire. Veuillez réessayer.", "error");
            }
          };
          displayFormWithRetry();
          
          // Mettre à jour l'UI pour refléter l'état non connecté (formulaire affiché)
          updateAccountButton();
          updateUserUI();
        ...
        // Afficher le formulaire UNIQUEMENT si :
        // 1. Le backend dit que le profil n'est PAS complet (profileComplete !== true)
        // 2. OU c'est un nouvel utilisateur (isNewUser === true)
        // 3. OU les données essentielles (username, photo) sont manquantes
        // ET le profil n'est pas déjà complet dans localStorage (fallback)
        // NOTE: isBackendUserEmpty est déjà géré ci-dessus
        const shouldShowForm = (isNewUser || !profileComplete || !hasUsername || !hasProfilePhoto) && !isProfileCompleteFromStorage;
        ...
        if (shouldShowForm) {
          ...
          // MAIS: Si backendUser est vide, c'est un problème de sérialisation, on DOIT afficher le formulaire
          if (isBackendUserEmpty) {
            console.log('⚠️ backendUser est vide malgré profileComplete === true - Affichage formulaire (problème sérialisation)', {
              profileComplete: profileComplete,
              isBackendUserEmpty: isBackendUserEmpty,
              userEmail: user.email
            });
          ...
          // CAS 2: Utilisateur EXISTANT avec profil complet ET données disponibles → Connexion directe à MapEvent
          // IMPORTANT: Ne JAMAIS afficher le formulaire si profileComplete === true ET données disponibles
          ...
          if (syncData.user && !isBackendUserEmpty) {
            currentUser = {
              ...currentUser,
              ...syncData.user,
...
function openLoginModal() {
  ...
  // SUPPRIMER TOUS LES CHAMPS EMAIL EXISTANTS pour éviter l'autofill
  // Le navigateur peut détecter des champs email même s'ils sont masqués
  const existingEmailInputs = document.querySelectorAll('input[type="email"], input[id*="email"], input[name*="email"]');
  existingEmailInputs.forEach(input => {
    if (input.id !== 'pub-email' && !input.closest('#publish-modal-inner')) {
      // Ne pas toucher aux champs en dehors de la modal de connexion
      return;
    }
    // Masquer temporairement pour éviter la détection
    if (input.closest('#publish-modal-inner')) {
  ...
  const html = `
    <div style="padding:40px;max-width:450px;margin:0 auto;text-align:center;" autocomplete="off" data-form-type="other">
      <!-- Logo et titre -->
      ...
      <div style="display:flex;flex-direction:column;gap:12px;margin-bottom:24px;">
        ...
        <!-- Bouton Email - Affiche les champs email/password au clic -->
        <!-- Utiliser un texte différent pour éviter la détection par le navigateur -->
        <button onclick="showEmailLoginForm()" style="width:100%;padding:14px 20px;border-radius:12px;border:2px solid rgba(255,255,255,0.1);background:linear-gradient(135deg,rgba(255,255,255,0.1),rgba(255,255,255,0.05));color:#fff;display:flex;align-items:center;justify-content:center;gap:12px;font-weight:600;font-size:15px;cursor:pointer;transition:all 0.3s;backdrop-filter:blur(10px);" onmouseover="this.style.borderColor='rgba(0,255,195,0.5)';this.style.background='linear-gradient(135deg,rgba(0,255,195,0.2),rgba(59,130,246,0.2))';" onmouseout="this.style.borderColor='rgba(255,255,255,0.1)';this.style.background='linear-gradient(135deg,rgba(255,255,255,0.1),rgba(255,255,255,0.05))';" data-email-button="true">
          <span style="font-size:20px;">📧</span>
          <span>Connexion avec Email</span>
        </button>
      </div>
      
      <!-- Connexion par email - CRÉÉE DYNAMIQUEMENT UNIQUEMENT AU CLIC SUR "EMAIL" -->
      <!-- Ne rien mettre ici - le formulaire sera créé dynamiquement -->
  ...
  // DÉSACTIVER L'AUTOFILL DU NAVIGATEUR de manière agressive
  // Le navigateur peut détecter des patterns même si les champs ne sont pas visibles
  const modalContent = document.querySelector('#publish-modal-inner div[style*="padding:40px"]');
  if (modalContent) {
    modalContent.setAttribute('autocomplete', 'off');
    modalContent.setAttribute('data-form-type', 'other');
    // Ajouter un style pour masquer les suggestions d'autofill
    modalContent.style.setProperty('--autofill-color', 'transparent');
  }
  
  // S'ASSURER qu'aucun champ email n'existe dans le DOM de la modal
  // Supprimer tous les champs email existants dans la modal
  setTimeout(() => {
    const emailInputsInModal = modal.querySelectorAll('input[type="email"], input[id*="email" i], input[name*="email" i]');
    emailInputsInModal.forEach(input => {
      if (input.closest('#publish-modal-inner')) {
        input.remove(); // Supprimer complètement du DOM
      }
    });
  }, 50);
  
  // NE PAS focus sur l'email par défaut - les champs sont masqués
  // NE PAS créer de champs email dans le DOM initial
}

// Fonction pour afficher le formulaire email au clic sur le bouton "Email"
...
function showEmailLoginForm() {
  ...
  let emailForm = document.getElementById("email-login-form");
  
  if (!emailForm) {
    // Créer le formulaire dynamiquement
    const formHtml = `
      <div id="email-login-form" style="margin-top:24px;" autocomplete="off">
        ...
        <!-- CHAMP FACTICE pour tromper le navigateur (masqué) -->
        <input type="email" name="fake_email" autocomplete="off" style="position:absolute;left:-9999px;opacity:0;pointer-events:none;tabindex:-1;" tabindex="-1" aria-hidden="true">
        
        <div style="margin-bottom:16px;text-align:left;">
          <label style="display:block;font-size:13px;font-weight:600;color:var(--ui-text-main);margin-bottom:8px;">📧 Email</label>
          <input type="email" id="login-email" autocomplete="off" placeholder="votre@email.com" style="width:100%;padding:12px 16px;border-radius:10px;border:2px solid rgba(255,255,255,0.1);background:rgba(15,23,42,0.5);color:var(--ui-text-main);font-size:14px;transition:all 0.3s;" onfocus="this.style.borderColor='rgba(0,255,195,0.5)';this.style.background='rgba(15,23,42,0.8)';" onblur="this.style.borderColor='rgba(255,255,255,0.1)';this.style.background='rgba(15,23,42,0.5)';">
        </div>
    ...
    // Insérer le formulaire après les boutons de connexion sociale (avant le bouton Annuler)
    const cancelButton = modal.querySelector('button[onclick="closePublishModal()"]');
    if (cancelButton && cancelButton.parentNode) {
      cancelButton.parentNode.insertBefore(document.createRange().createContextualFragment(formHtml), cancelButton);
    } else {
      // Fallback : ajouter à la fin du modal
      const modalContent = modal.querySelector('div[style*="padding:40px"]');
      if (modalContent) {
        modalContent.insertAdjacentHTML('beforeend', formHtml);
      } else {
        modal.insertAdjacentHTML('beforeend', formHtml);
      }
    }
  } else {
    // Si le formulaire existe déjà, juste l'afficher
    emailForm.style.display = "block";
  }
  
  // Focus sur l'email après affichage
  setTimeout(() => {
...
window.showEmailLoginForm = showEmailLoginForm;
...
async function performLogin() {
  ...
  // Validation manuelle de l'email (puisque nous utilisons type="text" pour éviter l'autofill)
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!emailRegex.test(email)) {
    showNotification("⚠️ Veuillez entrer une adresse email valide", "warning");
    const emailInput = document.getElementById("mapevent-user-email-input") || document.getElementById("login-email");
    if (emailInput) {
      emailInput.focus();
    }
    return;
...
function updateAccountButton() {
  ...
  if (accountBtn && avatarEl && nameEl) {
      ...
      if (currentUser.isLoggedIn) {
        // PRIORITÉ ABSOLUE: username uniquement (pas le nom/prénom Google, pas l'email)
        const displayName = currentUser.username || "Compte";
    ...
    <div style="padding:0;max-width:500px;margin:0 auto;">
      ...
      <div style="background:linear-gradient(135deg,#0f172a 0%,#1e293b 50%,#0f172a 100%);padding:20px;border-radius:16px 16px 0 0;position:relative;">
        <div style="display:flex;align-items:center;gap:16px;">
          ...
          <div style="flex:1;">
            <div style="font-size:18px;font-weight:800;color:#fff;margin-bottom:2px;">${currentUser.username || 'Utilisateur'}</div>
            <!-- Email et adresse postale masqués pour la confidentialité (sauvegardés mais pas affichés publiquement) -->
          </div>
        </div>
      </div>
...
function logout() {
  ...
  // Réinitialiser complètement currentUser
  currentUser = {
    id: null,
    email: '',
    username: '',
  ...
  try {
    ...
    console.warn('⚠️ Erreur lors du nettoyage des tokens:', e);
  }
  
  // Nettoyer currentUser dans localStorage (mais garder les autres données si possible)
  try {
    localStorage.removeItem('currentUser');
  } catch (e) {
    console.warn('⚠️ Erreur lors du nettoyage de currentUser:', e);
  }
  
  // Mettre à jour l'UI immédiatement
  updateAccountButton(); // Remet "Compte" au lieu du nom
  updateUserUI();
  ...
  console.log('✅ Déconnexion terminée - Bouton compte réinitialisé');
}
