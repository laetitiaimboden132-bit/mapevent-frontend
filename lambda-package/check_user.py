...
Utilise les mêmes variables d'environnement que le backend
"""
import os
import sys
import psycopg2

# Ajouter le répertoire parent au path pour importer depuis backend
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

# Importer get_db_connection depuis le backend
try:
    from backend.main import get_db_connection
except ImportError:
    # Fallback: créer la connexion directement
...
def check_user_email(email):
    ...
    if not conn:
        ...
        print("💡 Assurez-vous que DATABASE_URL est défini dans votre environnement")
        return
    ...
    try:
        ...
        for i, row in enumerate(rows, 1):
            user_id, user_email, user_email_canonical, username, first_name, last_name, created_at, profile_photo_url, google_sub, profile_complete = row
            ...
            print(f"Profil complet: {profile_complete or False}")
            print()
